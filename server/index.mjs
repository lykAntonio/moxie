...
/* ---------- DeepSeek 流式（用于进度展示）---------- */
function buildStreamMessages(p) {
  const lengthDesc = LENGTH_MAP[p.length] || LENGTH_MAP.medium;
  const platformNote = PLATFORM_NOTE[p.platform] || PLATFORM_NOTE.wechat;
  const sys =
    "你是资深内容主笔「墨写」，擅长为内容平台撰写高质量中文文章。" +
...
第二行单独写：===ARTICLE===
然后是完整的 Markdown 正文（用 ## 二级标题，可用列表/表格/引用增强可读性；不要插入任何图片；不要再重复标题）
...
const REWRITE_MODES = {
  ...
  humanize:
    "去除 AI 写作痕迹，让文字更像真人手写。具体要求：" +
    "① 删除空泛的填充语与开场白（如'值得注意的是''总的来说''在当今……的时代'）；" +
    "② 打破公式化结构，避免三段排比、刻意的二元对比、强行升华与金句；" +
    "③ 去掉夸大意义的措辞（如'标志着''彰显了''奠定了基础''深刻反映'）与以'-ing/……着'结尾的虚假深度；" +
    "④ 变化句子长短与节奏，少用破折号堆砌；" +
